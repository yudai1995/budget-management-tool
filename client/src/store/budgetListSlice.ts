    ...
    reducers: {
        getBudget: (state, action) => {
            ...
            action.payload.budgetData.forEach((data: Budget) => {
                const newItem = new Budget(data.id, data.amount, data.balanceType, data.content, data.date, data.categoryId)
 ...
 * @return {number} 合計値
 */
export const sumAmount = <T extends RootState | Budget[]>(data: T, type?: BalanceType) => {
    let dataList: Budget[]
    if (isState(data)) {
 ...
 * @return {number} 合計値
 */
export const getTargetDateList = <T extends RootState | Budget[]>(data: T, targetDate: [number, number, number?], targetDay?: string) => {
    let dataList: Budget[]
    if (isState(data)) {
        dataList = data.budgetList.data
    } else {
    ...
    if (targetDay) {
        ...
        return dataList.filter((data) => getYear(data.date) === targetDate[0].toString() && getMonth(data.date) === targetDate[1].toString())
    }
}
 ...
 * @return {number} 合計値
 */
export const getTargetAnnulaList = <T extends RootState | Budget[]>(data: T, target: Date) => {
    let dataList: Budget[]
    if (isState(data)) {
        dataList = data.budgetList.data
    } else {
 ...
 */
export const getFormatRecentBudgetList = <T extends RootState | Budget[]>(budgetList: T) => {
...
// actionをexport
export const { getBudget, addBudget, deleteBudget } = budgetListSlice.actions
// state情報をexport
