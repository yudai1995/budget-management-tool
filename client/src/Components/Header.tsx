...
import { getIsLoginAuth, getLoginUser, setLogout } from '../store/LoginStateSlice'
...
export const Header: React.FC = () => {
    ...
    useScrollPosition(
        ({ prevPos, currPos }) => {
            ...
            const shouldBeStyle = {
                transition: `all ${isVisible ? '20ms' : '20ms'} ${isVisible ? 'ease-in' : 'ease-out'}`,
            }
                    ...
                    <div className={styles.headerTitleWrapper}>
                        <h1 className={styles.headerTitle}>
                            <NavLink to="/" className={styles.logo}>
                                <img src={Logo} alt="家計簿管理" width="24" height="24" />
                                家計簿管理
                            </NavLink>
                        </h1>
                        {isLogin ? (
                            <div className={`${styles.loginWrapper} sp-only`}>
                                <p className={`${styles.loginUser}`}>
                                    {loginUser ? `${guest['guest-en'] === loginUser ? guest['guest-ja'] : loginUser}さん` : '未ログイン'}
                                </p>
