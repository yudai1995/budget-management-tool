...
フルスプリント自律モードで UI/UX 変更を含む PBI を実装する場合、`apps/sandbox/AGENTS.md` の Sandbox-First ルールを以下のように適用する。
...
> 判断に迷う場合は自律実行を一時停止し、ユーザーに「Sandbox レビューが必要か」を確認してから再開すること。
...
**すべての Issue に priority ラベルを付与すること。付け忘れは `auto-priority.yml` が P2 を自動補完するが、正確な priority は起票時に設定すること。**

| ラベル | 基準 | 目安 |
|--------|------|------|
| `priority: P0` | 本番障害・データ損失・セキュリティ脆弱性。今すぐ対応必須 | 即日 |
| `priority: P1` | 主要機能が使えない・今スプリントで実装すべき重要機能 | 今スプリント |
| `priority: P2` | 標準バックログ。近いうちに対応（デフォルト値） | 次スプリント |
| `priority: P3` | あれば嬉しい機能・軽微な改善。余裕があれば対応 | 将来 |

**AI エージェントが `gh issue create` を実行する際は、必ず `--label "priority: P?"` を含めること。**
省略した場合は `auto-priority.yml` が P2 を付与するが、意図した優先度を明示することを原則とする。
