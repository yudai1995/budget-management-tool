...
| 2 | `.github/ai-agent-workflow.md` | 必須プロセス・テスト要件・プロダクト判断基準 |
| 3 | `.github/pull-request-instructions.md` | PR 生成の論理ルール |
| 4 | `.github/commit-message-instructions.md` | コミットメッセージ規約 |
| 5 | `.github/coding-conventions.md` | コーディング規約（TS/BE/FE/アイコン） |
| 6 | `.github/multi-agent-roles.md` | マルチエージェント役割・Serena MCP ルール |
| 7 | `.github/PRD/product_requirements.md` | プロダクト要件・優先度 |
| 8 | `.github/design/` | UI/UX・設計ドキュメント |
| — | `.github/skills/` | スキル定義の SSOT（Claude/Cursor/Copilot 共通） |
| — | `.github/hooks/` | フックスクリプトの SSOT（Claude/Cursor/Copilot 共通） |
...
> **⚠️ 重要: このファイルのセクション 5（SSOT マップ）は、Claude / Cursor / Serena など全 AI エージェントが
...
- **要件変更のたびに `.github/PRD/` を更新する**（実装だけ変えてドキュメントを放置しない）
...
4. **スプリントゴール・仮説・計画ポイントを導出する**（PBI選定後に必ず実施）:
   ...
   - 承認後、`velocity-log.json` の当スプリントエントリに `goal`, `hypothesis`, `planned_points` を**必ず**記録する
...
1. 完了 PBI の一覧とサイクルタイムを集計（`velocity-log.json` から取得）し、達成率を計算する:
   - `達成率 = total_points / planned_points × 100`（`planned_points` が null の場合は「計測データなし」と表示）
...
## 8. スプリントエージェントプロトコル（Pattern A: Claude Code CLI as Sprint Agent）
...
6. **スプリントクロージング**:
   ...
   - velocity-log.json 同期確認（最大 3 分待機）
   - **velocity-log.json 手動補記**: `update-velocity.yml` は PR 本文の最初の `Closes #N` のみ処理する。複数 PBI を 1 PR でクローズした場合は、残りの PBI を手動で velocity-log.json に追記して main にコミットする
...
1. **スプリント番号を採番する**（最初に必ず実行）:
   ...
   - 変更をコミット・プッシュして main に反映する（`chore(sprint): スプリント #N を開始する`）
   - 以降このスプリントで作成する**すべての PR 本文に `Sprint: N`** を含める
2. `.github/velocity-log.json` から直近 3スプリントの平均ベロシティを計算する（実績がない場合は初期値 6pt）
...
1. **PBI 選択 & 開始タイムスタンプ記録**: `in-progress` → `sprint-backlog` 最優先 → 自動選定の順で対象 Issue を特定する。`in-progress` ラベル付与後、即座に以下のコメントを Issue に投稿する：
   ...
   ```
2. **ブランチ作成**: `.github/ai-agent-workflow.md` の `/branch` 手順を実行する
3. **翻訳**: Issue の内容を「ユーザーの意図 → 技術課題 → 修正対象ファイル」へ翻訳する
4. **影響調査**: 関連ファイル・型定義・依存関係を調査する
5. **実装**: コードを修正・追加する
6. **自己検証**: lint + build → テスト追加・更新 → `pnpm test:unit` 全件パス
7. **コミット**: Conventional Commits 形式、本文日本語
8. **プッシュ**: `git push -u origin {ブランチ名}`
9. **PR 作成 & ラベル更新**:
   - `.github/pull-request-instructions.md` に従って PR を作成する
...
2. `.github/velocity-log.json` から直近 3スプリントのベロシティ実績・size 別平均サイクルタイムを出力する
...
**背景**: このリポジトリでは Claude Code / Cursor / GitHub Copilot の複数 AI ツールを併用する。
...
2. **各 AI ツールには薄いラッパーを配置する**
   - Claude Code → `.claude/skills/{skill-name}.md`（1行: 「`.github/skills/X.md` を読んで実行」）
