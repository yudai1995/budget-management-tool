...
2. **作業終了時**: PR がマージされた瞬間、以下を Issue に自動コメント：
   - サイクルタイム（開始から完了までの実経過時間）
   - PR サマリー（タイトル・変更ファイル数）
   - レトロスペクティブ記入を促すリマインダー
...
2. `.github/retrospective-template.md` に基づいてレトロスペクティブを生成
3. 対象 Issue にコメントとして投稿
...
1. 完了 PBI の一覧とサイクルタイムを集計
2. レトロスペクティブを `.github/retrospective-template.md` に基づいて生成
3. 次スプリントへの持ち越し PBI を整理
4. ベロシティトレンド（直近3スプリント）を表示
