...
export class TypeORMExpenseRepository implements IExpenseRepository {
    ...
    async findById(id: string): Promise<Expense | null> {
        const dataModel = await this.dataSource.getRepository(BudgetDataModel).findOne({ where: { id } });
    ...
    async remove(id: string): Promise<void> {
        await this.dataSource.createQueryBuilder().delete().from(BudgetDataModel).where('id = :id', { id }).execute();
    }
}
